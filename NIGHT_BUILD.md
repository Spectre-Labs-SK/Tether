...
5. **Token Exhaustion Protocol:** If you run out of tokens or hit a hard usage limit and must pause, write a concise handoff in `TETHER_BUILD_JOURNAL.md` with exact current state, next action, and verification status. Do not assume another agent will take over automatically.
