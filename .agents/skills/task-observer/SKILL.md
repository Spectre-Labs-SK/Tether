...
with a AGENTS.md instruction or harness-level hook (see Recommended
...
This skill is already activated via AGENTS.md. The dual-layer setup is in place:
description-level triggers + a structural instruction in AGENTS.md that fires
...
2. **Inventory all skills.** Scan `.Codex/skills/` for all SKILL.md files.
...
4. Check AGENTS.md for task-observer activation instruction (already confirmed present).
...
This skill runs in **Codex** with full file system access. The full
persistent-storage workflow applies. No handoff doc mode needed.

All skill files are in `.Codex/skills/{name}/SKILL.md`.
